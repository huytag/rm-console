<template>
  <div class="assets-page min-h-full p-6 font-inter">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      <!-- Total Assets -->
      <div class="stat-card card-blue p-6 rounded-2xl border border-main flex flex-col justify-between h-[160px] relative overflow-hidden group">
        <div class="flex justify-between items-start z-10">
          <p class="text-[10px] font-black uppercase tracking-[0.2em] text-dim">Tổng tài sản</p>
          <div class="w-10 h-10 rounded-xl flex items-center justify-center bg-blue-500/10 text-blue-500 group-hover:scale-110 transition-transform">
      ...
      <!-- New/Good Assets -->
      <div class="stat-card card-emerald p-6 rounded-2xl border border-main flex flex-col justify-between h-[160px] group">
        <div class="flex justify-between items-start">
          <p class="text-[10px] font-black uppercase tracking-[0.2em] text-dim">Tài sản mới/tốt</p>
          <div class="w-10 h-10 rounded-xl flex items-center justify-center bg-emerald-500/10 text-emerald-500 group-hover:scale-110 transition-transform">
      ...
      <!-- Maintenance Needed -->
      <div class="stat-card card-amber p-6 rounded-2xl border border-main flex flex-col justify-between h-[160px] group">
        <div class="flex justify-between items-start">
          <p class="text-[10px] font-black uppercase tracking-[0.2em] text-dim">Cần bảo trì</p>
          <div class="w-10 h-10 rounded-xl flex items-center justify-center bg-amber-500/10 text-amber-500 group-hover:scale-110 transition-transform">
      ...
      <!-- Broken Assets -->
      <div class="stat-card card-rose p-6 rounded-2xl border border-main flex flex-col justify-between h-[160px] group">
        <div class="flex justify-between items-start">
          <p class="text-[10px] font-black uppercase tracking-[0.2em] text-dim">Tài sản hỏng</p>
    ...
    <!-- Filter Bar -->
    <div class="flex items-center gap-4 mb-6 bg-card/40 p-2 rounded-2xl border border-main">
      <div class="flex items-center gap-2 pl-4 border-r border-main pr-4">
        <span class="text-[10px] font-black uppercase text-dim whitespace-nowrap">Tòa nhà:</span>
        <el-select v-model="filters.building_id" placeholder="Tất cả tòa nhà" class="theme-select-mini" style="width: 160px;">
          <el-option label="Tất cả tòa nhà" :value="null" />
          <el-option v-for="b in buildings" :key="b.id" :label="b.name" :value="b.id" />
        </el-select>
      </div>
      <div class="flex items-center gap-2 border-r border-main pr-4">
        <span class="text-[10px] font-black uppercase text-dim whitespace-nowrap">Phòng:</span>
        <el-select v-model="filters.room_id" placeholder="Tất cả phòng" class="theme-select-mini" style="width: 150px;">
          <el-option label="Tất cả phòng" :value="null" />
          <el-option v-for="r in rooms" :key="r.id" :label="`Phòng ${r.room_number}`" :value="r.id" />
        </el-select>
      </div>
      <div class="flex items-center gap-2 border-r border-main pr-4">
        <span class="text-[10px] font-black uppercase text-dim whitespace-nowrap">Tình trạng:</span>
        <el-select v-model="filters.condition" placeholder="Tất cả tình trạng" class="theme-select-mini" style="width: 180px;">
    ...
    <div class="rounded-2xl border border-main overflow-hidden shadow-2xl bg-table">
        ...
        <table class="w-full text-sm border-collapse">
          <thead class="bg-header">
            <tr class="border-b border-main">
              ...
              <th class="px-6 py-5 text-left text-[11px] font-black uppercase tracking-widest text-dim">Tên tài sản</th>
              <th class="px-6 py-5 text-left text-[11px] font-black uppercase tracking-widest text-dim">Loại tài sản</th>
              <th class="px-6 py-5 text-left text-[11px] font-black uppercase tracking-widest text-dim">Phòng</th>
              <th class="px-6 py-5 text-center text-[11px] font-black uppercase tracking-widest text-dim">Tình trạng</th>
              <th class="px-6 py-5 text-right text-[11px] font-black uppercase tracking-widest text-dim">Giá mua</th>
              <th class="px-6 py-5 text-left text-[11px] font-black uppercase tracking-widest text-dim">Mô tả</th>
          ...
          <tbody v-if="!loading" class="bg-table">
            ...
            >
              <td class="px-6 py-5">
                <span class="font-bold text-xs text-id">#TS-{{ String(row.id).padStart(4, '0') }}</span>
              </td>
              <td class="px-6 py-5">
              ...
              <td class="px-6 py-5">
                <div class="flex flex-col">
                  <span class="text-main font-bold text-sm">{{ row.room?.room_number || 'Kho tổng' }}</span>
                  <span v-if="row.room?.building?.name" class="text-[10px] font-bold text-dim" style="font-size: 13px;">{{ row.room.building.name }}</span>
                </div>
              </td>
              ...
              <td class="px-6 py-5">
                <p class="text-xs text-dim line-clamp-1 max-w-[200px]" :title="row.description">{{ row.description || 'Chưa có mô tả' }}</p>
              </td>
              <td class="px-6 py-5 text-center">
                <div class="flex items-center justify-center gap-3">
                  <button class="action-btn btn-edit" title="Chỉnh sửa" @click="editAsset(row)">
      ...
      <div class="px-6 py-4 flex items-center justify-between bg-header border-t border-main">
        <span class="text-[11px] font-bold text-dim uppercase tracking-widest">
          Hiển thị 1 - {{ paginatedAssets.length }} trong tổng số <span class="text-main">{{ filteredAssets.length }}</span> tài sản
        </span>
      ...
      <el-form :model="form" :rules="rules" ref="formRef" label-position="top" class="mt-2">
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <el-form-item label="Giá mua (VNĐ)" prop="purchase_price" required>
            <el-input v-model.number="form.purchase_price" placeholder="Giá trị...">
        ...
        <el-form-item label="Mô tả chi tiết" prop="description">
          <el-input v-model="form.description" type="textarea" :rows="3" placeholder="Thông số kỹ thuật, ghi chú bảo hành..." />
        </el-form-item>
      </el-form>
...
import { 
  Box, Tools, CircleCheck, CircleClose, Search, Plus, 
  View, Edit, Delete, Bell, QuestionFilled, Warning, 
  ArrowLeft, ArrowRight, Top, Bottom, Wallet
} from '@element-plus/icons-vue'

// ========== MOCK DATA ==========
const mockAssets = [
  { id: 1029, name: 'Điều hòa Daikin 1.5HP', category: 'Điện lạnh', room: { id: 201, room_number: 'Phòng 201', building: { id: 1, name: 'Tòa nhà A - Mỹ Đình' } }, condition: 'new', purchase_price: 12500000, description: 'Bảo hành chính hãng 2 năm...' },
  { id: 1035, name: 'Tủ lạnh Samsung 200L', category: 'Gia dụng', room: { id: 104, room_number: 'Phòng 104', building: { id: 2, name: 'Tòa nhà B - Cầu Giấy' } }, condition: 'good', purchase_price: 7200000, description: 'Mới mua từ tháng 01/2023' },
  { id: 982, name: 'Máy nước nóng Ariston', category: 'Điện dân dụng', room: { id: 302, room_number: 'Phòng 302', building: { id: 1, name: 'Tòa nhà A - Mỹ Đình' } }, condition: 'fair', purchase_price: 3500000, description: 'Cần vệ sinh lọc nước định kỳ' },
  { id: 1144, name: 'Nệm Vạn Thành 1m8', category: 'Nội thất', room: { id: 405, room_number: 'Phòng 405', building: { id: 3, name: 'Sunrise Tower' } }, condition: 'poor', purchase_price: 4800000, description: 'Có vết ố, lò xo hơi yếu' },
  { id: 741, name: 'Bếp hồng ngoại Sunhouse', category: 'Gia dụng', room: null, condition: 'broken', purchase_price: 1200000, description: 'Vỡ mặt kính, không lên nguồn' },
]

const maintenanceHistory = [
  { title: 'Thay vòi sen - Phòng 101', description: 'Hoàn thành bởi: Nguyễn Văn A • 2 giờ trước', time: 'MỚI' },
  { title: 'Vệ sinh máy lạnh - Phòng 305', description: 'Đang xử lý • Dự kiến xong lúc 15:00', time: '14:30' },
  { title: 'Kiểm tra đường điện - Tòa nhà B', description: 'Đã trễ hạn • Cần nhân viên kỹ thuật gấp', time: 'HÔM QUA' },
]
...
const rules = { 
  ...
  category: [{ required: true, message: 'Vui lòng chọn loại tài sản', trigger: 'change' }],
  ...
  purchase_price: [{ required: true, message: 'Vui lòng nhập giá mua', trigger: 'blur' }],
}
...
const filteredAssets = computed(() => {
  return assets.value.filter(a => {
    ...
    const matchesCondition = !filters.condition || a.condition === filters.condition
    const matchesBuilding = !filters.building_id || a.room?.building?.id === filters.building_id
    const matchesRoom = !filters.room_id || (a.room?.id || a.room_id) === filters.room_id
...
const fetchData = async () => {
  loading.value = true
  try {
    const response = await api.get('/assets', { params: { page: currentPage.value, per_page: pageSize.value } })
...
const deleteAsset = async (asset) => {
  try {
    await ElMessageBox.confirm('Xóa tài sản này khỏi hệ thống?', 'Cảnh báo', { type: 'warning' })
