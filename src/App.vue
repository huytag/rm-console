        ...
        <div class="header-right">
            ...
            <el-popover placement="bottom" :width="300" trigger="click">
              ...
              <div class="notif-panel">
                <div class="notif-header">
                  <span>Thông báo</span>
                  <el-button link type="primary" size="small" @click="markAllRead">Đọc tất cả</el-button>
                </div>
                <div class="notif-list" v-loading="notifLoading">
                  <div v-for="notif in notifications" :key="notif.id" :class="['notif-item', { unread: !notif.read_at }]">
                    <p class="notif-data">{{ notif.data.message }}</p>
                    <span class="notif-time">{{ formatDate(notif.created_at) }}</span>
                  </div>
                  <p v-if="notifications.length === 0" class="notif-empty">Không có thông báo mới</p>
                </div>
              </div>
            </el-popover>
          </el-badge>
          ...
          <el-dropdown @command="handleCommand">
            <span class="user-dropdown">
              <el-avatar :size="32" src="https://cube.elemecdn.com/0/88/03b0d39583f48206768a7534e55bcpng.png" class="user-avatar" />
              {{ user?.name || 'Admin' }}
            </span>
            <template #dropdown>
              <el-dropdown-menu>
...
import { 
  HomeFilled, Office, Document, Wallet, User, Setting, 
  Box, UserFilled, DataAnalysis, Bell
